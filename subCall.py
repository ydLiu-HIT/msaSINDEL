...
def makeCandidate(chrName, alignment, RNAME, REF, rS, SEQ, shift, source):
    ...
    for m in re.finditer(cigarRe, CIGAR):
        ...
        if m.group(2) in ("M", "="): # M, X, =
            ...
            for _ in range(op_l):
                ...
                if sig == True:
                    rseq = REF[refPos-rS]; qseq = SEQ[quePos-1]
            ...
            if sig == True:
                rseq = REF[refPos-rS]; qseq = SEQ[quePos-1:quePos+op_l]
            ...
            if sig == True:
                rseq = REF[refPos-rS:refPos+op_l+1-rS]; qseq = SEQ[quePos-1]
...
def realignWithQuality(QUERY, QUAL, Haplot, var, Param, amplifier, extflank):
    if DEBUG:
    ...
    for m in range(len(QUERY)):
        ...
        for hap in Haplot:
            TS, TL, LH = Param[od]; od += 1;
            alignment = ksw_aligner(qseq, hap)
...
def realignToHaplotype(QUERY, QUAL, Haplot, var): 
    Param = []
    ...
    likeliScore = realignWithQuality(QUERY, QUAL, Haplot, var, Param, amplifier, extflank)

    return likeliScore

def most_likely_genotype(CandidateList, QUERY, QUAL):
    for var in CandidateList:
        ...
        likeliScore = realignToHaplotype(QUERY, QUAL, Haplot, var)
        if DEBUG:
        ...
        #del likeliScore; del GenotypeLikeliHood; del real_probs; del predictions
...
def mergeCoVar(canList, refseq, sid):
    variantList = list()
    Lcan = len(canList)
    if Lcan == 0: return 

    if Lcan == 1:
        var = createVariantCandidate(canList[0][:-1], refseq, sid, canList[0][-1])
        variantList.append(var)
    else:
        canList = sorted(canList, key=lambda x:x[1])
        ins1 = canList[0]; t = 1
        while t < Lcan:
            ins2 = canList[t]
            if ins2[1] != ins1[1]:
                var = createVariantCandidate(ins1[:-1], refseq, sid, ins1[-1])
                variantList.append(var)
                ins1 = canList[t]
                t += 1
                ...
                var = createVariantCandidate(ins1[:-1], refseq, sid, ins1[-1], ins2[-1])
                variantList.append(var)
